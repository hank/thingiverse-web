...
TypingText.prototype.run = function() {
  ...
  if(typeof this.origText == "undefined") {
    setTimeout("document.getElementById('" + this.element.id + "').typingText.run()", Math.floor(Math.random() * this.interval));	// We haven't finished loading yet.  Have patience.
    return;
  }
...
//  this.origText = this.origText.replace(/<([^<])*>/, "");     // Strip HTML from text.
  if(this.currentChar < this.origText.length) {
    ...
    this.currentChar++;
    setTimeout("document.getElementById('" + this.element.id + "').typingText.run()", Math.floor(Math.random() * this.interval));
  } else {
	this.currentText = "";
	this.currentChar = 0;
        this.running = false;
